...
import { searchPluginFixture } from 'e2e/utils/common';

const pythonVersions = [['3.7'], ['3.8', '3.9']];
test.describe('Plugin filter tests', () => {
  pythonVersions.forEach(async (pythonVersion) => {
    test.only(`should filter by python versions ${pythonVersion.toString()}`, async ({
      page,
      viewport,
    }) => {
