...
class TestInstallActivity:
    ...
    @pytest.fixture()
    def install_activity_table(self, create_dynamo_table):
        with mock_dynamodb():
            yield create_dynamo_table(install_activity._InstallActivity,
                                      'install-activity')

    def test_batch_write(self, install_activity_table, verify_table_data):
