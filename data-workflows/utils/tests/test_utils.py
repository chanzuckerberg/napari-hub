...
class TestParameterStore:
    ...
    def test_get_last_updated_timestamp_valid(self, aws_credentials):
        ...
        self._client.put_parameter(
            Name=EXPECTED_PARAMETER_NAME,
            Value=PARAMETER_STORE_VALUE,
            Type="SecureString",
        )
    ...
    def test_get_last_updated_timestamp_invalid(self, aws_credentials):
        self._client = boto3.client("ssm")
        self._client.put_parameter(
            Name=EXPECTED_PARAMETER_NAME, Value="foo-bar", Type="SecureString"
        )
    ...
    def test_set_last_updated_timestamp(self, aws_credentials):
        ...
        response = self._client.get_parameter(
            Name=EXPECTED_PARAMETER_NAME, WithDecryption=True
        )
        actual = json.loads(response["Parameter"]["Value"]).get(
            "last_activity_fetched_timestamp"
        )
        assert actual == timestamp
