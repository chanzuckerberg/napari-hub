...
class TestHandle:
    ...
    def _setup(self, monkeypatch):
        ...
        monkeypatch.setattr(
            handler, "ParameterStoreAdapter", self._parameter_store_adapter_call
        )
        self._update_install_activity_call = Mock()
        monkeypatch.setattr(
            activity.processor,
            "update_install_activity",
            self._update_install_activity_call,
        )
        self._update_github_activity_call = Mock()
        monkeypatch.setattr(
            activity.processor,
            "update_github_activity",
            self._update_github_activity_call,
        )
    ...
    def test_handle_valid_activity_event(self, monkeypatch):
        parameter_store_adapter = Mock(
            get_last_updated_timestamp=lambda: LAST_UPDATED_TIMESTAMP
        )
        self._parameter_store_adapter_call.return_value = parameter_store_adapter
        monkeypatch.setattr(
            utils.utils, "get_current_timestamp", lambda: CURRENT_TIMESTAMP
        )
        ...
        self._verify(1)
        self._update_install_activity_call.assert_called_once_with(
            LAST_UPDATED_TIMESTAMP, CURRENT_TIMESTAMP
        )
        self._update_github_activity_call.assert_called_once_with(
            LAST_UPDATED_TIMESTAMP, CURRENT_TIMESTAMP
        )
        parameter_store_adapter.set_last_updated_timestamp.assert_called_once_with(
            CURRENT_TIMESTAMP
        )
