...
class GitHubActivityType(Enum):
    def __new__(
        cls, timestamp_formatter, type_id_formatter, projection, sort, expiry_formatter
    ):
    ...
    MONTH = (
        ...
        "name, month",
        lambda ts: int((ts + relativedelta(months=14)).timestamp()),
    )
    TOTAL = (
        lambda timestamp: None,
        "TOTAL:{repo}",
    ...
    def format_to_type_identifier(
        self, repo_name: str, timestamp: Optional[datetime]
    ) -> str:
        return self.type_identifier_formatter.format(
            repo=repo_name, timestamp=timestamp
        )

    def to_expiry(self, timestamp: Optional[datetime]) -> Optional[int]:
