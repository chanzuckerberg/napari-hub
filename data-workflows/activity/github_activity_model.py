...
class GitHubActivityType(Enum):
    def __new__(cls, timestamp_formatter, type_identifier_formatter,
                query_projection, query_sorting):
...
def transform_and_write_to_dynamo(data: dict[str, List],
                                  activity_type: GitHubActivityType) -> None:
    """
    ...
    """
    LOGGER.info(
        f'Starting item creation for github-activity type={activity_type.name}')
    ...
    for repo, github_activities in data.items():
        ...
        for activity in github_activities:
            ...
            item = {
                ...
                "type_timestamp": activity_type.format_to_type_identifier(repo,
                                                                          identifier_timestamp),
    ...
    LOGGER.info(
        f'Items github-activity type={activity_type.name} count={count}')
    LOGGER.info(
        f'Transform and write to github-activity type={activity_type.name} timeTaken={duration}ms')
