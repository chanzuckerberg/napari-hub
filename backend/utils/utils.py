...
def parse_manifest(manifest: Optional[dict] = None):
    ...
    manifest_attributes['display_name'] = manifest['display_name'] or ''
    manifest_attributes['npe2'] = not (manifest['npe1_shim'] or False)
    manifest_contributions = manifest['contributions']

    readers = manifest_contributions['readers'] or []
    if readers:
    ...
    writers = manifest_contributions['writers'] or []
    if writers:
        manifest_attributes['plugin_types'].append('writer')
        writer_file_extensions = set()
        writer_save_layers = set()
        for writer in writers:
            filename_extensions = writer['filename_extensions'] or []
            layer_types = writer['filename_extensions'] or []
    ...
    if manifest_contributions['themes']:
        manifest_attributes['plugin_types'].append('theme')
    if manifest_contributions['widgets']:
